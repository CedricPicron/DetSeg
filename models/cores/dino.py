"""
Neck module from Mask DINO with auxiliary modules and functions.
...
class DinoNeck(nn.Module):
    ...
    def __init__(
        ...
        """
        Args:
            in_channels: list with input channels
            in_strides: list with input strides
        ...
        """
        super().__init__()
        self.in_ids = list(range(len(in_channels)))
        self.feature_strides = in_strides
        self.feature_channels = in_channels
        self.feature_order = feature_order

        self.transformer_in_ids = transformer_in_ids if 'high2low' else transformer_in_ids[::-1]
        transformer_in_channels = [in_channels[i] for i in self.transformer_in_ids]
        self.transformer_feature_strides = [in_strides[i] for i in self.transformer_in_ids]

        self.maskdino_num_feature_levels = num_feature_levels
        self.total_num_feature_levels = total_num_feature_levels
        self.common_stride = common_stride

        self.transformer_num_feature_levels = len(transformer_in_ids)
    ...
    def forward(self, features, masks=None, **kwargs):
        ...
        if self.total_num_feature_levels > self.transformer_num_feature_levels:
            smallest_feat = features[self.transformer_in_ids[self.low_resolution_index]].float()
            _len_srcs = self.transformer_num_feature_levels
            for lvl in range(_len_srcs, self.total_num_feature_levels):
                if lvl == _len_srcs:
                    src = self.input_proj[lvl](smallest_feat)
                else:
                    src = self.input_proj[lvl](srcsl[-1])
                srcsl.append(src)
                posl.append(self.pe_layer(src))
        srcsl = srcsl[::-1]
        # Reverse feature maps
        for module_id, feat_id in enumerate(self.transformer_in_ids[::-1]):
            x = features[feat_id].float()  # deformable detr does not support half precision
            srcs.append(self.input_proj[module_id](x))
        ...
        for module_id, feat_id in enumerate(self.in_ids[:self.num_fpn_levels][::-1]):
            x = features[feat_id].float()
            lateral_conv = self.lateral_convs[module_id]
            output_conv = self.output_convs[module_id]
            cur_fpn = lateral_conv(x)
            y = cur_fpn + F.interpolate(out[self.high_resolution_index], size=cur_fpn.shape[-2:], mode="bilinear",
                                        align_corners=False)
            y = output_conv(y)
            out.append(y)
