...
class BLAv3(nn.Module):
    """
    Class implementing the BLA (Bidirectional Local Attention) module (experimental version 3).

    Same as experimental version 2, but with sequential top-down and bottom-up attention mechanisms.
    ...
    def forward_update(self, in_feat_maps, layer_id, **kwargs):
        ...
        # Perform top-down attention if requested
        if self.td_attn:
            feat_map2 = in_feat_maps[-1]
            interpolation_kwargs = {'mode': 'bilinear', 'align_corners': True}

            zip_list = [range(num_maps-2, -1, -1), in_feat_maps[-2::-1], self.td_q_in_projs[layer_id][::-1]]
            zip_list = [*zip_list, self.td_kv_in_projs[layer_id][::-1], self.attn_scalings[-2::-1]]
            zip_list = [*zip_list, self.num_heads_list[-2::-1], self.td_out_projs[layer_id][::-1]]

            for i, feat_map1, q_in_proj, kv_in_proj, scale, num_heads, out_proj in zip(*zip_list):
                f1, H1, W1 = feat_map1.shape[1:]
                H2, W2 = feat_map2.shape[2:]
                ...
                top_down_attn_map = F.pad(top_down_attn_map, (0, pW, 0, pH), mode='replicate')
                top_down_attn_map = F.interpolate(top_down_attn_map, size=(H1+pH, W1+pW), **interpolation_kwargs)
                top_down_attn_map = top_down_attn_map[:, :, :H1, :W1]

                delta_feat_maps[i] += top_down_attn_map
                feat_map2 = feat_map1 + top_down_attn_map

        # Perform bottom-up attention if requested
        if self.bu_attn:
            feat_map0 = in_feat_maps[0]

            zip_list = [range(num_maps), in_feat_maps[1:], self.bu_q_in_projs[layer_id]]
            zip_list = [*zip_list, self.bu_kv_in_projs[layer_id], self.attn_scalings[1:], self.num_heads_list[1:]]
            zip_list = [*zip_list, self.bu_out_projs[layer_id]]
            ...
            for i, feat_map1, q_in_proj, kv_in_proj, scale, num_heads, out_proj in zip(*zip_list):
                ...
                attn_weights = F.softmax(torch.matmul(query_map, key_map), dim=-1)
                weighted_map = torch.sum(attn_weights * value_map, dim=-1).view(batch_size, H1, W1, -1)
                bottom_up_attn_map = out_proj(weighted_map.permute(0, 3, 1, 2))

                delta_feat_maps[i+1] += bottom_up_attn_map
                feat_map0 = feat_map1 + bottom_up_attn_map
...
def build_bla(args):
    ...
    if args.bla_version == 'main':
    ...
    elif args.bla_version in ['v1', 'v2', 'v3']:
        ...
        elif args.bla_version == 'v3':
            bla = BLAv3(num_layers, in_feat_sizes, out_feat_sizes, attn_dict, ffn_dict)
