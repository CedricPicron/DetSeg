"""
Cocofy LVIS annotations by removing annotations of categories not present in COCO.

The implementation is based on the one provided in Detectron2.
"""
...
def cocofy_lvis(input_filename, output_filename):
    """
    Filter LVIS instance segmentation annotations to remove all categories that are not included in COCO. The new json
    files can be used to evaluate COCO AP using `lvis-api`. The category ids in the output json are the incontiguous
    COCO dataset ids.
...
if __name__ == "__main__":
    input_filename = 'lvis_v0.5_val.json'
    output_filename = 'lvis_v0.5_val_cocofied.json'

    print(f"Start COCOfing LVIS annotations from {input_filename}.")
    cocofy_lvis(input_filename, output_filename)
    print(f"The LVIS annotations from {input_filename} are COCOfied and stored in {output_filename}.")
